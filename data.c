...
struct group* new_group()
{
    ...
    if(!g)
        return NULL;
    g->students=malloc(8*sizeof(struct student));
    g->g_size=0;
    g->g_max=8;
    return g;
}
...
{
    struct student *e=realloc(g->students, 2*g->g_max*sizeof(struct student));
    if(!e)
    {
        return 1;
    }
...
{
    struct student *e=realloc(g->students, g->g_max*sizeof(struct student)/2);
    if(!e)
    {
        return 1;
    }
